...
class Population(object):
    ...
    def write(self):
        ...
        if self.individuals[0].ploidy == 'N':
            ...
            pop = np.empty_like(genA, dtype=np.object)
    ...
    def stats(self):
        ...
        missing = [np.sum(np.logical_or(x.genomeA == -9, x.genomeB == -9))/float(len(x.genomeA)) for x in self.individuals]
        
        #print("Heterokaryotic ratio {0}".format("\t".join(["{0:.3f}".format(x) for x in  het])))
        print("Mean heterokaryotic ratio {0}".format(np.mean(het)))
        
        #print("Missing SNPs {0}".format("\t".join(["{0:.3f}".format(x) for x in  missing])))
        print("Mean missing SNPs {0}".format(np.mean(missing)))
...
class Simulation(object):
    ...
    def __init__(self, reference=None, random_seed=None, ploidy='N'):
        self.reference = reference
    ...
    def missingData(self, popA, rate_mean, rate_sd):
        """Simulate missing data in the individuals of popA such 
        that a fraction rate of the SNP sites are set to -9-9"""
        n_errs = np.array(np.floor(len(self.reference['loci']) * np.abs(self.rng.normal(rate_mean, rate_sd, self.populations[popA].n_individuals))), dtype=np.int)

        for i,indiv in enumerate(self.populations[popA].individuals):
            position = self.rng.choice(np.arange(0, len(indiv.genomeA)), n_errs[i], replace=False)
            indiv.genomeA[position] = -9
            indiv.genomeB[position] = -9
